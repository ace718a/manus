...
function req(id)
{
	...
	if($("#"+id+" [name='agree1']") && $("#"+id+" [name='agree1']").prop("checked")==false){
		...
		alert(chkTitle + "에 동의 하셔야 이용가능합니다.\n확인후 진행해주시길 바랍니다.");
		return false;
	}

	var hp1 = $("#"+id+" [name='hp1']").val();
	var hp2 = $("#"+id+" [name='hp2']").val();
	var hp3 = $("#"+id+" [name='hp3']").val();
	var authnum = $("#"+id+" [name='authnum']").val();

	var hp = hp1 + hp2 + hp3;

	var is_submit = auth_hp_req(hp, authnum);

	if(is_submit) {
		$('#btn_submit').attr('disabled', true);
		$('#btn_submit').html('<i class="fa fa-spinner fa-spin fa-lg"></i> 등록중입니다');
			...
			$.ajax({
				...
				success : function(json){
					if(json.res == true){
						...
						alert("무료 방문 견적 비교 신청이 완료되었습니다.\n업체와 매칭이 이루어지는 대로 알림톡으로 안내 드리겠습니다!");
...
function auth_hp_req(hp, authnum) {
	var is_check = false;

	$.ajax({
		type : 'POST',
		url : 'auth_api.php',
		data : { mode : 'save', hp : hp, authnum : authnum },
		dataType: 'json',
		async : false,
		error: function(xhr, status, error){
			alert('서버통신에 문제가 있습니다. 잠시후 다시 시도해 주세요.');
